...
class Network(object):
    ...
    def decoder(w_bar, config, training, C, reuse=False, actv=tf.nn.relu, channel_upsample=960):
        ...
        with tf.variable_scope('decoder', reuse=reuse):
            ...
            ups = tf.layers.separable_conv2d(ups, input_attributes.DEPTH, kernel_size=7, strides=1, padding='VALID')
            print("Decoder upsampled vector shape - ", ups.shape)
            out = tf.nn.tanh(ups)
    ...
    def discriminator(x, config, training, reuse=False, actv=tf.nn.leaky_relu, use_sigmoid=False, ksize=4):
        ...
        with tf.variable_scope('discriminator', reuse=reuse):
            ...
            c1 = tf.layers.separable_conv2d(x, 64, kernel_size=ksize, strides=2, padding='same', activation=actv)
            ...
            out = tf.layers.separable_conv2d(c4, 1, kernel_size=ksize, strides=1, padding='same')
...
def residual_block(x, n_filters, kernel_size=3, strides=1, actv=tf.nn.relu, training=True):
    ...
    res = tf.layers.separable_conv2d(res, filters=n_filters, kernel_size=kernel_size, strides=strides,
...
def conv_block(x, filters, kernel_size=[3,3], strides=2, padding='same', actv=tf.nn.relu):
    ...
    in_kwargs = {'center':True, 'scale': True}
    x = tf.layers.separable_conv2d(x, filters, kernel_size, strides=strides, padding=padding, activation=None)
    x = tf.contrib.layers.instance_norm(x, **in_kwargs)
    x = actv(x)
    return x
